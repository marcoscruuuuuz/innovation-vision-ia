...
CREATE INDEX IF NOT EXISTS idx_p2p_failover_operations_dvr_time
ON p2p_failover_operations(dvr_id, started_at DESC);

CREATE UNIQUE INDEX IF NOT EXISTS uq_p2p_failover_operation_active_dvr
ON p2p_failover_operations(dvr_id)
WHERE completed_at IS NULL AND state NOT IN ('COMMITTED','ROLLED_BACK','FAILED');
