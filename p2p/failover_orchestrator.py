...
def set_operation(cur, operation_id: UUID, state: str, step: str, *, error: str | None = None, details: dict | None = None, completed: bool = False):
    cur.execute(
        """UPDATE p2p_failover_operations SET state=%s,step=%s,error=%s,details=details || %s::jsonb,updated_at=now(),completed_at=CASE WHEN %s THEN now() ELSE completed_at END WHERE id=%s""",
...
def run_failover(dvr_id: UUID, payload: FailoverRequest):
    ...
    try:
        with db() as conn, conn.cursor() as cur:
            set_operation(cur,op["id"],"OPENING_DESTINATION","open_destination"); conn.commit()
        opened=api("POST",f"{P2P_SUPERVISOR_URL}/v1/p2p/dvrs/{dvr_id}/open",{"actor_type":payload.actor_type,"reason":f"failover:{op['id']}","wine_worker_id":str(destination_worker["id"]),"allow_parallel":True})
        destination_session=opened["session"]; probe_frames=int(opened["probe"]["frame_count"])
        with db() as conn, conn.cursor() as cur:
            ...
            cur.execute("SELECT id,channel FROM cameras WHERE dvr_id=%s AND enabled=true ORDER BY channel",(dvr_id,)); cameras=cur.fetchall(); conn.commit()
        if not cameras: raise RuntimeError("DVR has no enabled cameras to switch")
        with db() as conn, conn.cursor() as cur:
            set_operation(cur,op["id"],"SWITCHING_ROUTES","switch_routes",details={"camera_count":len(cameras)}); conn.commit()
        for camera in cameras:
            channel=int(camera["channel"]); local_uri=f"rtsp://127.0.0.1:{int(destination_session['rtsp_local_port'])}/cam/realmonitor?channel={channel}&subtype=0"
            api("POST",f"{STREAM_BROKER_URL}/v1/routes/switch",{"camera_id":str(camera["id"]),"p2p_session_id":str(destination_session["id"]),"local_uri":local_uri,"probe_frames":probe_frames,"reason":f"failover:{op['id']}","actor_type":payload.actor_type})
        ...
        with db() as conn, conn.cursor() as cur:
            ...
            cur.execute("""SELECT count(*) AS total FROM stream_routes r JOIN cameras c ON c.id=r.camera_id WHERE c.dvr_id=%s AND c.enabled=true AND r.state='ACTIVE' AND r.deactivated_at IS NULL AND r.p2p_session_id=%s""",(dvr_id,destination_session["id"])); verified=int(cur.fetchone()["total"])
            cur.execute("SELECT count(*) AS total FROM cameras WHERE dvr_id=%s AND enabled=true",(dvr_id,)); expected=int(cur.fetchone()["total"])
            if verified!=expected: raise RuntimeError(f"route verification mismatch: {verified}/{expected}")
            conn.commit()
        if source:
            with db() as conn, conn.cursor() as cur:
                set_operation(cur,op["id"],"CLOSING_SOURCE","close_source"); conn.commit()
        ...
        with db() as conn, conn.cursor() as cur:
            ...
            cur.execute("INSERT INTO audit_logs(actor_type,action,object_type,object_id,metadata) VALUES (%s,'p2p.failover.committed','dvr',%s,%s::jsonb)",(payload.actor_type,str(dvr_id),json.dumps({"operation_id":str(op["id"]),"destination_session_id":str(destination_session["id"]),"routes":verified}))); conn.commit()
        return {"status":"COMMITTED","plan":plan,"destination_session":destination_session,"routes":verified}
    except Exception as exc:
        with db() as conn, conn.cursor() as cur:
            set_operation(cur,op["id"],"ROLLING_BACK","rollback",error=str(exc),details={"switched_cameras":switched_cameras}); conn.commit()
        if destination_session:
            for camera_id in reversed(switched_cameras):
                try:
                    api("POST",f"{STREAM_BROKER_URL}/v1/routes/rollback",{"camera_id":camera_id,"expected_current_session_id":str(destination_session["id"]),"reason":f"failover_rollback:{op['id']}","actor_type":"SYSTEM"})
                except Exception as rollback_exc: rollback_errors.append(f"route:{camera_id}:{rollback_exc}")
            try:
            ...
            except Exception as rollback_exc: rollback_errors.append(f"destination_close:{rollback_exc}")
        final_state="ROLLED_BACK" if not rollback_errors else "FAILED"
        with db() as conn, conn.cursor() as cur:
            set_operation(cur,op["id"],final_state,"rollback_complete" if not rollback_errors else "rollback_incomplete",error=str(exc),details={"rollback_errors":rollback_errors},completed=True)
            cur.execute("INSERT INTO audit_logs(actor_type,action,object_type,object_id,metadata) VALUES ('SYSTEM','p2p.failover.rollback','dvr',%s,%s::jsonb)",(str(dvr_id),json.dumps({"operation_id":str(op["id"]),"error":str(exc),"rollback_errors":rollback_errors}))); conn.commit()
        raise HTTPException(status_code=502,detail={"message":"failover rolled back" if not rollback_errors else "failover rollback incomplete; operator intervention required","operation_id":str(op["id"]),"error":str(exc),"rollback_errors":rollback_errors}) from exc
