...
def reserve_one(cur, protocol: str, start: int, end: int, dvr_id: UUID, worker_id: UUID | None):
    cur.execute("UPDATE p2p_port_leases SET state='EXPIRED', released_at=now() WHERE state IN ('RESERVED','BOUND') AND released_at IS NULL AND expires_at < now()")
    for port in range(start, end + 1):
        cur.execute("""INSERT INTO p2p_port_leases(port, protocol, dvr_id, wine_worker_id, expires_at)
                       VALUES (%s,%s,%s,%s,%s) ON CONFLICT DO NOTHING
                       RETURNING id, port, protocol, lease_token, state, leased_at, expires_at""",
                    (port, protocol, dvr_id, worker_id, datetime.now(timezone.utc) + timedelta(seconds=LEASE_SECONDS)))
        row = cur.fetchone()
        if row:
            return row
    raise HTTPException(status_code=503, detail=f"no free {protocol} ports")
...
def wine_heartbeat(payload: WorkerHeartbeat):
    with db() as conn, conn.cursor() as cur:
        cur.execute("""INSERT INTO wine_workers(worker_key, host, pid, state, cpu_percent, ram_mb, active_sessions, last_heartbeat_at)
                       VALUES (%s,%s,%s,%s,%s,%s,%s,now())
                       ON CONFLICT(worker_key) DO UPDATE SET host=excluded.host, pid=excluded.pid, state=excluded.state,
                       cpu_percent=excluded.cpu_percent, ram_mb=excluded.ram_mb, active_sessions=excluded.active_sessions,
                       last_heartbeat_at=now() RETURNING *""",
...
def list_wine():
    with db() as conn, conn.cursor() as cur:
        cur.execute("SELECT w.*, CASE WHEN w.last_heartbeat_at >= now() - interval '90 seconds' THEN true ELSE false END AS heartbeat_fresh FROM wine_workers w ORDER BY worker_key")
        return cur.fetchall()
...
def reserve_ports(payload: LeaseRequest):
    with db() as conn, conn.cursor() as cur:
        ...
        if not dvr["enabled"]: raise HTTPException(status_code=409, detail="DVR disabled")
        if dvr["connection_mode"] != "intelbras_p2p": raise HTTPException(status_code=409, detail="DVR is not configured for intelbras_p2p")
        sdk = reserve_one(cur, "SDK_TCP", SDK_PORT_START, SDK_PORT_END, payload.dvr_id, payload.wine_worker_id)
        rtsp = reserve_one(cur, "RTSP_TCP", RTSP_PORT_START, RTSP_PORT_END, payload.dvr_id, payload.wine_worker_id)
        cur.execute("INSERT INTO audit_logs(actor_type, action, target_type, target_id, details) VALUES (%s,'p2p.ports.reserve','dvr',%s,%s::jsonb)", (payload.actor_type, str(payload.dvr_id), '{"status":"reserved"}'))
        conn.commit(); return {"sdk": sdk, "rtsp": rtsp}
...
def dvr_tunnel_history(dvr_id: UUID):
    with db() as conn, conn.cursor() as cur:
        cur.execute("""SELECT s.id, s.state, s.sdk_local_port, s.rtsp_local_port, s.started_at, s.ended_at,
                       ...
                       WHERE s.dvr_id=%s ORDER BY s.started_at DESC LIMIT 200""", (dvr_id,))
        return cur.fetchall()
...
def rotate_tunnel(dvr_id: UUID, payload: RotateRequest):
    with db() as conn, conn.cursor() as cur:
        ...
        plan = {"dvr_id": str(dvr_id), "current_session": current, "steps": ["reserve destination SDK/RTSP ports","open new Intelbras P2P session","validate authentication","validate RTSP and multiple real frames","switch StreamBroker atomically","confirm heartbeat","close previous session and release old leases","persist audit/history"], "vendor_adapter_enabled": VENDOR_ADAPTER_ENABLED}
        if not payload.execute: return {"mode": "PLAN", "plan": plan}
        if not VENDOR_ADAPTER_ENABLED: raise HTTPException(status_code=409, detail={"message": "Intelbras vendor adapter is not installed/enabled; execution blocked", "plan": plan})
...
def scheduler_plan():
    with db() as conn, conn.cursor() as cur:
        cur.execute("""SELECT id, worker_key, state, cpu_percent, ram_mb, active_sessions, last_heartbeat_at,
