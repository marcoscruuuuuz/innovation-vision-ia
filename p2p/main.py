...
app = FastAPI(title="INNOVATION VISION P2P Supervisor", version="0.2.0")
...
def open_dvr_session(dvr_id: UUID, payload: SessionOpenRequest):
    ...
    with db() as conn, conn.cursor() as cur:
        ...
        dvr = cur.fetchone()
        if not dvr: raise HTTPException(status_code=404, detail="DVR not found")
        if not dvr["enabled"] or dvr["connection_mode"] != "intelbras_p2p": raise HTTPException(status_code=409, detail="DVR is not enabled for intelbras_p2p")
        missing = [k for k in ("serial_secret_ref", "username_secret_ref", "password_secret_ref") if not dvr.get(k)]
        if missing: raise HTTPException(status_code=409, detail={"message": "DVR secret references are incomplete", "missing": missing})
        cur.execute("SELECT id FROM p2p_sessions WHERE dvr_id=%s AND ended_at IS NULL AND state IN ('OPENING','ACTIVE','DEGRADED') LIMIT 1", (dvr_id,))
        if cur.fetchone(): raise HTTPException(status_code=409, detail="DVR already has an active/opening P2P session")
    ...
    try:
        opened = adapter.open_session(
            ...
            sdk_local_port=sdk["port"], rtsp_local_port=rtsp["port"], wine_worker_key=worker["worker_key"],
        )
        ...
        if vendor_session_ref:
            try: adapter.close_session(session_ref=vendor_session_ref)
            except VendorAdapterError: pass
        with db() as conn, conn.cursor() as cur:
    ...
    with db() as conn, conn.cursor() as cur:
        ...
        cur.execute("INSERT INTO audit_logs(actor_type, action, object_type, object_id, metadata) VALUES (%s,'p2p.session.active','dvr',%s,%s::jsonb)", (payload.actor_type, str(dvr_id), json.dumps({"session_id": str(session["id"]), "worker": worker["worker_key"], "frames": probe.frame_count})))
        conn.commit()
...
def list_active_leases(dvr_id: UUID | None = None):
    with db() as conn, conn.cursor() as cur:
        if dvr_id: cur.execute("SELECT * FROM p2p_port_leases WHERE released_at IS NULL AND dvr_id=%s ORDER BY leased_at DESC", (dvr_id,))
        else: cur.execute("SELECT * FROM p2p_port_leases WHERE released_at IS NULL ORDER BY leased_at DESC LIMIT 500")
        return cur.fetchall()
...
def list_sessions(dvr_id: UUID | None = None):
    with db() as conn, conn.cursor() as cur:
        if dvr_id: cur.execute("SELECT s.*, w.worker_key FROM p2p_sessions s LEFT JOIN wine_workers w ON w.id=s.wine_worker_id WHERE s.dvr_id=%s ORDER BY s.started_at DESC LIMIT 100", (dvr_id,))
        else: cur.execute("SELECT s.*, w.worker_key FROM p2p_sessions s LEFT JOIN wine_workers w ON w.id=s.wine_worker_id ORDER BY s.started_at DESC LIMIT 250")
        return cur.fetchall()
...
def rotate_tunnel(dvr_id: UUID, payload: RotateRequest):
    ...
    plan = {"dvr_id": str(dvr_id), "current_session": current, "reason": payload.reason, "steps": ["reserve destination ports","open destination vendor session","probe at least real frames","switch StreamBroker atomically","confirm heartbeat","close previous session","release old leases","persist audit/history"], "vendor_adapter": adapter.public_status()}
    if not payload.execute: return {"mode": "PLAN", "plan": plan}
    raise HTTPException(status_code=501, detail={"message": "automatic rotate execution requires StreamBroker switch contract; no unsafe partial rotation will be performed", "plan": plan})
