...
def reserve_one(cur, protocol: str, start: int, end: int, dvr_id: UUID, worker_id: UUID | None):
    cur.execute(
        """
        UPDATE p2p_port_leases
           SET state='EXPIRED', released_at=now()
         WHERE state IN ('RESERVED','BOUND') AND released_at IS NULL AND expires_at < now()
        """
    )
    for port in range(start, end + 1):
        cur.execute(
            """
            INSERT INTO p2p_port_leases(port, protocol, dvr_id, wine_worker_id, expires_at)
            VALUES (%s,%s,%s,%s,%s)
...
def list_wine():
    with db() as conn, conn.cursor() as cur:
        cur.execute(
            """
            SELECT w.*,
                   CASE WHEN w.last_heartbeat_at >= now() - interval '90 seconds' THEN true ELSE false END AS heartbeat_fresh
              FROM wine_workers w
             ORDER BY worker_key
            """
        )
...
def reserve_ports(payload: LeaseRequest):
    with db() as conn, conn.cursor() as cur:
        ...
        cur.execute(
            "INSERT INTO audit_logs(actor_type, action, target_type, target_id, details) VALUES (%s,'p2p.ports.reserve','dvr',%s,%s::jsonb)",
...
def scheduler_plan():
    with db() as conn, conn.cursor() as cur:
        ...
        return {"workers": workers, "policy": "lowest_load_score_first", "execution": "requires validated destination session"}
