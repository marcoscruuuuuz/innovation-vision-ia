...
def already_has_clip(job) -> bool:
    with db() as conn, conn.cursor() as cur:
        cur.execute("SELECT id FROM event_evidence WHERE event_log_id=%s AND media_type='clip' LIMIT 1", (job["event_log_id"],))
        found = bool(cur.fetchone())
        if found:
            cur.execute(
                "UPDATE evidence_clip_jobs SET status='DONE',completed_at=now(),retry_at=NULL,last_error=NULL WHERE id=%s",
                (job["id"],),
            )
            conn.commit()
        return found
...
def build_clip(job):
    ...
    with tempfile.TemporaryDirectory(prefix="vision-clip-") as tempdir:
        ...
        cmd = [
            ...
            "-c:v", "mpeg4", "-q:v", "5", "-movflags", "+faststart", "-y", str(output),
        ]
        subprocess.run(cmd, check=True, timeout=duration + 20, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        ...
        with db() as conn, conn.cursor() as cur:
            cur.execute(
                ...
                SELECT %s,%s,'clip',%s,%s
                WHERE NOT EXISTS (SELECT 1 FROM event_evidence WHERE event_log_id=%s AND media_type='clip')
                """,
                (job["event_log_id"], object_key, digest, len(data), job["event_log_id"]),
            )
            cur.execute(
                "UPDATE evidence_clip_jobs SET status='DONE',completed_at=now(),retry_at=NULL,last_error=NULL WHERE id=%s",
                (job["id"],),
