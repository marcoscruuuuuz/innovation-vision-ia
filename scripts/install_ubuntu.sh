...
install_nvidia() {
  ...
  case "$INSTALL_NVIDIA_TOOLKIT" in
    ...
    no) return 0 ;;
    auto)
      if has_nvidia_hardware || command -v nvidia-smi >/dev/null 2>&1; then
        should_install=yes
      fi
      ;;
  ...
  [[ "$should_install" == yes ]] || {
    log 'GPU NVIDIA não detectada; NVIDIA Container Toolkit não será instalado'
    return 0
  }
  ...
  if ! command -v nvidia-smi >/dev/null 2>&1 || ! nvidia-smi >/dev/null 2>&1; then
    if [[ "$INSTALL_NVIDIA_DRIVER" == yes ]]; then
      ...
      warn 'GPU NVIDIA detectada, mas nvidia-smi não está funcional. Instale/homologue o driver e reexecute o script. O toolkit será instalado, mas teste GPU ficará pendente.'
    fi
  fi
  ...
  if command -v nvidia-smi >/dev/null 2>&1 && nvidia-smi >/dev/null 2>&1; then
    log 'Driver NVIDIA funcional; runtime Docker configurado'
  else
    warn 'Toolkit configurado, mas validação GPU depende de reboot/driver funcional'
  fi
}
...
install_repository() {
  ...
  mkdir -p \
    "$VISION_ROOT/data/postgres" \
    "$VISION_ROOT/data/redis" \
    "$VISION_ROOT/data/minio" \
    "$VISION_ROOT/models" \
    "$VISION_ROOT/logs" \
    "$VISION_ROOT/backups" \
    "$VISION_ROOT/secrets/vendor/intelbras" \
    "$VISION_ROOT/secrets/cameras" \
    "$VISION_ROOT/secrets/evolution"
...
ExecStart=/usr/bin/docker compose --env-file ${VISION_ROOT}/.env up -d postgres redis minio prometheus node-exporter grafana
...
validate_installation() {
  ...
  if command -v nvidia-smi >/dev/null 2>&1 && nvidia-smi >/dev/null 2>&1; then
    docker info 2>/dev/null | grep -qi nvidia && log 'Runtime NVIDIA visível ao Docker' || warn 'runtime NVIDIA não apareceu no docker info; revisar configuração'
  fi
}
...
start_infrastructure() {
  ...
  cd "$VISION_ROOT"
  docker compose --env-file .env up -d postgres redis minio prometheus node-exporter grafana

  if command -v nvidia-smi >/dev/null 2>&1 && nvidia-smi >/dev/null 2>&1; then
    docker compose --env-file .env --profile gpu up -d dcgm-exporter || warn 'DCGM exporter não iniciou; revisar driver/runtime NVIDIA'
  fi
...
main() {
  ...
  if (( REBOOT_REQUIRED == 1 )); then
    warn 'Reboot necessário para concluir a instalação/ativação do driver NVIDIA. Após reiniciar, execute novamente este instalador para validar o runtime GPU.'
  fi
}
