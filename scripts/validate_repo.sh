...
required_files=(
  ...
  configs/event_catalog.yaml
  configs/p2p.example.yaml
  scripts/install_ubuntu.sh
  docs/01-ARCHITECTURE.md
  docs/02-DASHBOARD.md
  docs/03-EVENTS.md
  docs/04-P2P-WINE.md
  docs/05-CERTIFICATION.md
  docs/06-AI-IDE.md
  docs/07-DATA-SECURITY.md
  docs/08-INSTALLATION.md
  docs/09-IMPLEMENTATION-PHASES.md
  docs/10-ACCEPTANCE.md
)

for path in "${required_files[@]}"; do
  [[ -f "$path" ]] && ok "arquivo presente: $path" || fail "arquivo ausente: $path"
done
...
for path in "${required_dirs[@]}"; do
  [[ -d "$path" ]] && ok "domínio presente: $path" || fail "domínio ausente: $path"
done
...
while IFS= read -r -d '' script; do
  if bash -n "$script"; then
    ...
    fail "erro de sintaxe shell: ${script#./}"
  fi
...
if command -v shellcheck >/dev/null 2>&1; then
  if shellcheck scripts/*.sh; then
    ...
    fail 'shellcheck encontrou problemas'
  fi
else
...
PY
  then
    ok 'YAML e catálogo de eventos'
  else
    rc=$?
    if [[ $rc -eq 2 ]]; then
      warn 'python3-yaml não instalado; YAML não validado'
    else
      fail 'YAML/catálogo inválido'
    fi
  fi
fi
...
if unknown_top:
    raise SystemExit(f'chaves top-level Compose não reconhecidas pelo validador local: {sorted(unknown_top)}')
allowed_service = {
    'image','restart','environment','volumes','healthcheck','networks','command','ports',
    'profiles','deploy','pid','cap_add','depends_on','user','entrypoint','working_dir',
    'read_only','tmpfs','security_opt','devices','runtime','ipc','shm_size','ulimits'
}
for name, svc in compose['services'].items():
    ...
    if 'image' in svc and not isinstance(svc['image'], str):
        raise SystemExit(f'image inválida em {name}')
print(f'Compose estrutural OK: {len(compose["services"])} serviços')
PY
  then
    ok 'estrutura local do Docker Compose'
  else
    rc=$?
    if [[ $rc -eq 2 ]]; then
      warn 'python3-yaml não instalado; estrutura Compose não validada localmente'
    else
      fail 'estrutura local do Docker Compose inválida'
    fi
  fi
fi
...
if git ls-files | grep -E '(^|/)(\.env|.*\.(key|pem|p12|pfx|dll|exe|onnx|engine|pt|pth))$' >/dev/null 2>&1; then
  fail 'arquivo potencialmente sensível/binário proibido está versionado'
else
  ok 'nenhum segredo/binário proibido versionado'
fi
...
for name in tracked:
    ...
    for lineno, line in enumerate(text.splitlines(), 1):
        ...
        if not m:
            continue
        value = m.group(2)
        if 'CHANGE_ME' in value or value.startswith('${') or value.startswith('$(') or value.startswith('secret://'):
            continue
...
PY
  then
    ok 'scanner heurístico de segredos'
  else
    warn 'scanner heurístico encontrou possíveis segredos; revisar /tmp/vision-secret-scan.txt'
  fi
fi
